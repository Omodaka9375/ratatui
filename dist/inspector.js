...
function savePos(root, collapsed) {
    try {
...
function makeDraggable(root, header, getCollapsed) {
...
export function mountInspector(parent = document.body) {
    injectStyles();
    ...
    header.addEventListener("dblclick", (e) => {
        if (!e.target.closest("button")) {
            const collapseBtn = root.querySelector(".rui-collapse");
            collapseBtn?.click();
        }
    });
    const knobsEl = root.querySelector(".rui-knobs");
    const bindsEl = root.querySelector(".rui-binds");
    const knobRows = new Map();
    ...
    const addKnob = (k) => {
        ...
        labelSpan.className = "rui-label";
        labelSpan.textContent = k.label;
        const valSpan = document.createElement("span");
        valSpan.className = "rui-val";
        valSpan.textContent = fmt(k.get());
