...
export function slider(el: HTMLInputElement, k: Knob<number>): () => void {
  const onInput = (e: Event) => k.set(parseFloat((e.target as HTMLInputElement).value));
  el.addEventListener("input", onInput);
  const unbind = bind(k, (v) => v, (v) => { if (parseFloat(el.value) !== v) el.value = String(v); }, `ui:${k.label}`);
  return () => { el.removeEventListener("input", onInput); unbind(); };
}

export function textInput(el: HTMLInputElement, k: Knob<string>): () => void {
  const onInput = (e: Event) => k.set((e.target as HTMLInputElement).value);
  el.addEventListener("input", onInput);
  const unbind = bind(k, (v) => v, (v) => { if (el.value !== v) el.value = v; }, `ui:${k.label}`);
  return () => { el.removeEventListener("input", onInput); unbind(); };
}

export function toggle(el: HTMLInputElement, k: Knob<boolean>): () => void {
  const onChange = (e: Event) => k.set((e.target as HTMLInputElement).checked);
  el.addEventListener("change", onChange);
  const unbind = bind(k, (v) => v, (v) => { if (el.checked !== v) el.checked = v; }, `ui:${k.label}`);
  return () => { el.removeEventListener("change", onChange); unbind(); };
}

export function colorInput(el: HTMLInputElement, k: Knob<string>): () => void {
  const onInput = (e: Event) => k.set((e.target as HTMLInputElement).value);
  el.addEventListener("input", onInput);
  const unbind = bind(k, (v) => v, (v) => { if (el.value !== v) el.value = v; }, `ui:${k.label}`);
  return () => { el.removeEventListener("input", onInput); unbind(); };
...
export function select(el: HTMLSelectElement, k: Knob<string | number>): () => void {
...
export function textarea(el: HTMLTextAreaElement, k: Knob<string>): () => void {
  const onInput = (e: Event) => k.set((e.target as HTMLTextAreaElement).value);
  el.addEventListener("input", onInput);
  const unbind = bind(k, (v) => v, (val) => {
...
export function numberInput(el: HTMLInputElement, k: Knob<number>): () => void {
  const onInput = (e: Event) => k.set(parseFloat((e.target as HTMLInputElement).value) || 0);
  el.addEventListener("input", onInput);
  const unbind = bind(k, (v) => v, (val) => {
    if (parseFloat(el.value) !== val) el.value = String(val);
  }, `ui:${k.label}`);
  return () => { el.removeEventListener("input", onInput); unbind(); };
}

// editable(el, signal, editingSignal?) — inline editing of real page text.
export function editable(el: HTMLElement, s: Knob<string>, editing?: Knob<boolean> | Derived<boolean>): () => void {
  ...
  if (editing) {
    disposers.push(bind(editing, Boolean, (on) => {
      try { el.contentEditable = on ? "plaintext-only" : "false"; }
      catch { el.contentEditable = on ? "true" : "false"; }
...
export function editableImg(
  el: HTMLImageElement,
  s: Knob<string>,
  editing?: Knob<boolean> | Derived<boolean>
): () => void {
  const disposers = [
    bind(s, (v) => v, (v) => { if (el.src !== v) el.src = v; }, `img:${s.label}`),
  ];
  ...
  const onPaste = (e: ClipboardEvent) => {
    ...
    if (files && files.length > 0 && files[0].type.startsWith("image/")) {
      ...
      reader.onload = () => s.set(reader.result as string);
  ...
  const onDrop = (e: DragEvent) => {
    ...
    if (text && /^https?:\/\/.+/i.test(text)) s.set(text);
  };
...
export function editableVideo(
  el: HTMLVideoElement | HTMLIFrameElement | HTMLElement,
  s: Knob<string>,
  editing?: Knob<boolean> | Derived<boolean>
): () => void {
  ...
  const setSrc = (url: string) => {
    ...
    if (isIframe || isVideo) {
      if ((el as HTMLIFrameElement).src !== embedUrl) (el as HTMLIFrameElement).src = embedUrl;
    } else {
      // Generic element: use a child iframe
      let iframe = el.querySelector("iframe.rui-video-embed") as HTMLIFrameElement | null;
      if (!iframe) {
  ...
  const mediaEl = (): HTMLElement | null =>
    (isIframe || isVideo ? el : el.querySelector("iframe.rui-video-embed")) as HTMLElement | null;
