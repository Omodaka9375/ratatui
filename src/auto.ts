...
const ce = (e: { el: HTMLElement; on: boolean }) => {
  try { e.el.contentEditable = e.on ? "plaintext-only" : "false"; }
  catch { e.el.contentEditable = e.on ? "true" : "false"; }
};

function resolveAdapter(opts: Record<string, any>, getStoredToken: () => string | null): Adapter {
  ...
  switch (opts.adapter) {
    ...
    default:
      if (opts.endpoint) return restAdapter(opts.endpoint, { token });
...
function scanEditableText(
  ...
  document.querySelectorAll("[data-edit]").forEach((el) => {
    const name = (el as HTMLElement).dataset.edit;
    if (!name) return;
    signals[name] = knob((snap?.[name] ?? (el as HTMLElement).textContent!.trim()) as any, name);
    sc.add(editable(el as HTMLElement, signals[name], isEditing));
  });
}

function scanEditableImg(
  signals: Record<string, Knob<any>>,
  snap: any,
  isEditing: Derived<boolean>,
  sc: Scope,
  ...
  document.querySelectorAll("[data-edit-img]").forEach((el) => {
    const name = (el as HTMLElement).dataset.editImg;
    if (!name) return;
    const defaultSrc = (el as HTMLImageElement).src || (el as HTMLImageElement).dataset.src || "";
    signals[name] = knob((snap?.[name] ?? defaultSrc) as any, name);
    sc.add(editableImg(el as HTMLImageElement, signals[name], isEditing));
  });
}

function scanEditableVideo(
  signals: Record<string, Knob<any>>,
  snap: any,
  isEditing: Derived<boolean>,
  sc: Scope,
  ...
  document.querySelectorAll("[data-edit-video]").forEach((el) => {
    const name = (el as HTMLElement).dataset.editVideo;
    if (!name) return;
    const defaultSrc = (el as HTMLIFrameElement).src || (el as HTMLElement).dataset.src || "";
    signals[name] = knob((snap?.[name] ?? defaultSrc) as any, name);
    sc.add(editableVideo(el as HTMLElement, signals[name], isEditing));
  });
}

function scanEditableInput(
  signals: Record<string, Knob<any>>,
  snap: any,
  editing: Knob<string>,
  isEditing: Derived<boolean>,
  ...
  document.querySelectorAll("[data-edit-input]").forEach((input) => {
    ...
    const onInput = (e: Event) => {
      if (editing.get() !== "on") return; // locked for visitors
      s.set(isNum ? parseFloat((e.target as HTMLInputElement).value) || 0 : (e.target as HTMLInputElement).value);
    };
    el.addEventListener("input", onInput);
    sc.add(() => el.removeEventListener("input", onInput));
    sc.add(bind([s], (v) => v, (v) => {
      if (el.value !== String(v)) el.value = String(v);
      if (cssVar) document.documentElement.style.setProperty(cssVar, String(v));
    }, `ui:${name}`));
...
function scanEditableAttr(
  signals: Record<string, Knob<any>>,
  snap: any,
  sc: Scope,
) {
  document.querySelectorAll("[data-edit-attr]").forEach((el) => {
    (el as HTMLElement).dataset.editAttr!.split(";").forEach((pair) => {
      const [attr, name] = pair.split(":").map((s) => s.trim());
      if (!attr || !name) return;
      const s = (signals[name] =
        signals[name] ?? knob(snap?.[name] ?? (el as HTMLElement).getAttribute(attr) ?? "", name));
      sc.add(bind([s], (v) => v, (v) => {
        if ((el as HTMLElement).getAttribute(attr) !== v) (el as HTMLElement).setAttribute(attr, v);
      }, `attr:${name}→${attr}`));
    });
  });
}

function scanEditableList(
  signals: Record<string, Knob<any>>,
  snap: any,
  isEditing: Derived<boolean>,
  sc: Scope,
  ...
  document.querySelectorAll("[data-edit-list]").forEach((container) => {
    ...
    const fieldsOf = (el: Element) => {
      const o: Record<string, any> = {};
      (el as HTMLElement).querySelectorAll("[data-field]").forEach((f) => {
        const fieldEl = f as HTMLElement;
    ...
    sc.add(list(container as HTMLElement, s, (item, row) => {
      ...
      node.querySelectorAll("[data-field]").forEach((f) => {
        ...
        row.bind([item], (t: any) => t[fname], (v: any) => {
          if (fieldEl.textContent !== String(v)) fieldEl.textContent = String(v);
        }, `${name}.${fname}`);
        ...
        const onFieldInput = () => {
          ...
          s.update((a: any[]) => a.map((t: any) => t.id === item.get().id ? { ...t, [fname]: v } : t));
        };
      ...
      del.textContent = "×";
      del.title = "Remove";
      const onDel = () => s.update((a: any[]) => a.filter((t: any) => t.id !== item.get().id));
      del.addEventListener("click", onDel);
      row.add(() => del.removeEventListener("click", onDel));
      node.appendChild(del);
      return node;
    }, { key: (t: any) => t.id, label: name }));
    ...
    add.addEventListener("click", () => {
      ...
      (prototype as HTMLElement).querySelectorAll("[data-field]").forEach((f) => {
        const fieldEl = f as HTMLElement;
        blank[fieldEl.dataset.field!] = fieldEl.dataset.type === "number" ? 0 : "New " + (fieldEl.dataset.field || "");
      });
      s.update((a: any[]) => [...a, { id: Date.now(), ...blank }]);
    });
    (container as HTMLElement).after(add);
    sc.add(() => add.remove());
  });
}
...
function buildCmsBar(
  editing: Knob<string>,
  site: DraftApi<Record<string, Knob<any>>>,
  tokenCtl?: TokenCtl,
): () => void {
  const disposers: (() => void)[] = [];
  const on = (el: HTMLElement, event: string, fn: (e: Event) => void) => {
  ...
  let tokenSpan: HTMLSpanElement | null = null;
  ...
  if (tokenCtl) {
    ...
    const store = () => {
      ...
      if (!v) return;
      try { localStorage.setItem(tokenCtl.tokenKey, v); } catch { /* storage unavailable */ }
      tokenInput!.value = "";
      tokenSpan!.classList.remove("flash");
      renderToken();
    };
    on(saveBtn, "click", store);
    on(tokenInput, "keydown", (e) => { if ((e as KeyboardEvent).key === "Enter") store(); });
    on(stateBtn, "click", () => {
      try { localStorage.removeItem(tokenCtl.tokenKey); } catch { /* storage unavailable */ }
      renderToken();
      tokenInput!.focus();
    });
  ...
  disposers.push(bind([editing], (v) => v === "on",
    (on) => {
  ...
  disposers.push(bind([site.dirty, site.status],
    ...
    (a, b) => a.d === b.d && a.st === b.st));

  disposers.push(bind([site.conflict], (c) => !!c,
    (has) => { conflictSpan.hidden = !has; }, "→ cms:conflict"));
...
export async function autocms(userOptions: Record<string, any> = {}): Promise<AutocmsApi> {
  const tag = document.querySelector("script[data-ratatui-cms]") as HTMLScriptElement | null;
  ...
  if (tag?.dataset.token || tag?.dataset.ghToken) {
    console.warn(
      "[RatatUI CMS] data-token / data-gh-token are no longer supported and are ignored. " +
  ...
  let published: { snapshot: any; version: string | null } | null = null;
  try { published = await store.load(); }
  catch (e) { console.warn("[RatatUI CMS] load failed — starting from HTML defaults.", e); }
  const snap = published?.snapshot ?? null;

  // Scan DOM for editable annotations
  const signals: Record<string, Knob<any>> = {};
  ...
  if (opts.extend) await opts.extend(signals, ctx);

  // Own all scanned signals so dispose() cleans them up
  for (const s of Object.values(signals)) rootScope.own(s);
