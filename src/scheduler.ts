...
const queue = new Set<{ job: () => void; binding?: Binding }>();
let scheduled = false;

// Exported for testing/debugging — tracks bindings currently executing
export const activeBindings = new Set<Binding>();
const MAX_RECURSION_DEPTH = 100;
const chainDepth = new Map<string, number>();

export function schedule(job: () => void, binding?: Binding | null) {
      ...
      for (const { job, binding } of jobs) {
        ...
        if (depth > MAX_RECURSION_DEPTH) {
          if (binding) {
            ...
            console.error(
              `[RatatUI] Possible binding cycle detected [${binding.label}]. ` +
        ...
        if (binding) activeBindings.add(binding);
        try {
          job();
        } catch (e) {
          reportError(e as Error, { label: binding?.label ?? 'anon', phase: "scheduler" });
          console.error('[RatatUI] Job execution error:', e);
        } finally {
          if (binding) activeBindings.delete(binding);
